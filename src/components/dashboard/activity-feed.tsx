  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-[#0b0b0b] p-6">
      ...
      <div className="h-72 space-y-3 overflow-y-auto text-sm [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {loading ? <ListLoadingSkeleton rows={3} /> : items.length === 0 ? <p className="text-zinc-500">No completed tasks yet.</p> : items.map((item) => (
          <Link key={item.id} href={`/jobs/${item.id}`} className="flex items-start gap-3 rounded-lg border border-zinc-900 bg-zinc-950 p-3 hover:border-violet-900">
