...
function JobCard({ job, occurrences, onUpdate, onHistory }: { job: Job; occurrences?: Occurrence[]; onUpdate: (id: string, body: Record<string, string>) => Promise<void>; onHistory: (id: string) => Promise<void> }) {
  ...
  return (
    <article className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6">
      ...
      {occurrences && <div className="mt-3 space-y-2">{occurrences.length === 0 ? <p className="text-sm text-zinc-500">No runs have been scheduled yet.</p> : occurrences.map((item) => <div key={item.id} className="flex flex-wrap justify-between gap-2 rounded-lg bg-zinc-900 p-3 text-sm"><div><p className="text-zinc-200">{formatDate(item.scheduledFor, job.timezone)} · {item.status.replaceAll("_", " ")}</p>{item.reason && <p className="text-zinc-500">{item.reason}</p>}</div><div className="text-right text-zinc-400">{item.taskStatus && <p>Task: {item.taskStatus}</p>}{item.paymentStatus && <p>Payment: {item.paymentStatus}</p>}{item.taskId && <Link className="text-violet-400" href={`/tasks/${item.taskId}`}>View task</Link>}</div></div>)}</div>}
    </article>
  );
}
